...
for (file in csv_files) {
  ...
  # Extract the idp_name from the file name by removing prefixes and suffixes
  # Example file: "i_dat_dMRI_on_lacunar_IDP_trait.csv", extract "IDP_trait" as the idp_name
  idp_name <- sub(".*_dat_dMRI_on_lacunar_(.*)\\.csv", "\\1", file)
  ...
  write.csv(result, file = paste0(i, "_res_", gsub(" ", "_", idp_name), ".csv"), row.names = FALSE)
  
  # Check if any "Inverse variance weighted" method p-values are less than 0.05/288=0.00017361
  if (any(res$method == "Inverse variance weighted" & res$pval < 0.00017361)) {
    result$IDP <- idp_name  # Add IDP name column
    result$i <- i  # Add loop index column
  ...
  # Store pleiotropy test results and add IDP name column
  ...
  # Add loop index column, IDP, ivw_pval to steiger_df
  steiger_df$ivw_pval <- ivw_pval
  steiger_df$IDP <- idp_name
  steiger_df$i <- i
  # Append Steiger results to directionality_results dataframe
...
write.csv(heterogeneity_results, file = "heterogeneity_results_dMRI_on_Lacunar.csv", row.names = FALSE)
write.csv(pleiotropy_results, file = "pleiotropy_results_dMRI_on_Lacunar.csv", row.names = FALSE)
write.csv(directionality_results, file = "directionality_results_dMRI_on_Lacunar.csv", row.names = FALSE)
write.csv(significant_results, file = "significant_results_dMRI_on_Lacunar.csv", row.names = FALSE)